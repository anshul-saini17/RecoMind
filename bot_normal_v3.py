...
prompt_template = ChatPromptTemplate.from_messages([
    ("system", 
     "You are a helpful and knowledgeable assistant at Recogni which is a AI infrastructure company. "
     "Use the provided context below to answer the user’s question. "
     "Give elaborate responses until and unless it is specified to give SUMMARY of something"
     "If the context is insufficient or not relevant, fall back to your own general knowledge and ask the USER follow up questions"
     "Context:\n{context}"),
...
classification_prompt = PromptTemplate.from_template(
    "You are a smart assistant that determines if a user's message needs to search a document knowledge base.\n"
    "If the message is a factual, technical, or product-related question that may benefit from document context, say 'Yes'.\n"
    "If the message is casual, chit-chat, or clearly doesn’t need context, say 'No'.\n\n"
    "User message: {query}\n\n"
    "Should this query trigger a search in the knowledge base?\nAnswer 'Yes' or 'No' only."
)
...
def should_retrieve(query: str, llm) -> bool:
    ...
    #print(f"[Classifier Response] {response.content}")
...
def query_rag(query: str, db: FAISS, chat_history, container, model_name="gpt-4"):
    ...
    if should_retrieve(standalone_query, retriever_model):
